...
// Přidat, upravit, smazat dodavatele
...
import { Plus, Edit2, Trash2, X, ChevronDown, ChevronRight, Building2 } from 'lucide-react'
...
export default function SuppliersPage() {
  ...
  const [expandedSuppliers, setExpandedSuppliers] = useState<Set<string>>(highlightId ? new Set([highlightId]) : new Set())
  ...
  useEffect(() => {
    if (highlightId && filteredSuppliers.length > 0) {
      const index = filteredSuppliers.findIndex(item => item.id === highlightId)

      if (index !== -1) {
        const pageNumber = Math.floor(index / itemsPerPage) + 1
        setCurrentPage(pageNumber)

        setExpandedSuppliers(new Set([highlightId]))
  ...
  }, [highlightId, filteredSuppliers, itemsPerPage])
  ...
  useEffect(() => {
    ...
    if (filterContact) {
      filtered = filtered.filter(s =>
        (s.contact || '').toLowerCase().includes(filterContact.toLowerCase())
      )
    }
    ...
    if (filterPhone) {
      filtered = filtered.filter(s =>
        (s.phone || '').toLowerCase().includes(filterPhone.toLowerCase())
      )
    }
    ...
    if (filterWeb) {
      filtered = filtered.filter(s =>
        (s.website || '').toLowerCase().includes(filterWeb.toLowerCase())
      )
    }

    setFilteredSuppliers(filtered)
    setCurrentPage(1) // Reset stránky při změně filtrů
  }, [suppliers, filterName, filterContact, filterEmail, filterPhone, filterWeb])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editingSupplier) {
        // Upravit existujícího dodavatele
        ...
        if (response.ok) {
          alert('Dodavatel úspěšně upraven!')
        } else {
          alert('Nepodařilo se upravit dodavatele')
        }
      } else {
        ...
        if (response.ok) {
          alert('Dodavatel úspěšně přidán!')
        } else {
          alert('Nepodařilo se přidat dodavatele')
        }
      }

      // Znovu načti dodavatele
  ...
  async function handleDelete(supplier: Supplier) {
    if (!confirm(`Opravdu chceš smazat dodavatele "${supplier.name}"?`)) {
      return
    }
        ...
        <div className="text-center">
          <h1 className="text-2xl font-bold text-blue-600">
            ...
            <span className="text-sm font-normal text-gray-600 ml-3">
              (Zobrazeno <span className="font-semibold text-blue-600">{filteredSuppliers.length}</span> z <span className="font-semibold text-gray-700">{suppliers.length}</span>)
            </span>
          </h1>
        </div>
      </div>

      {/* Formulář pro přidání/úpravu dodavatele */}
                ...
                {formData.entityType === 'company' && (
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="https://www.dodavatel.cz"
                      className="bg-white border-blue-200 focus:border-blue-400 focus:ring-blue-400"
                    />
                  </div>
                )}
      ...
      <div ref={sectionRef} className="space-y-2">
        {suppliers.length === 0 ? (
          <div className="border rounded-lg p-12 text-center">
            <p className="text-gray-500 mb-4">Zatím nemáš žádné dodavatele</p>
            <p className="text-sm text-gray-400">Klikni na modrý panel nahoře pro přidání prvního dodavatele</p>
          </div>
        ) : (
          <>
            {/* Filtrační řádek */}
                ...
                return (
                  <div
                    ...
                    className={`border rounded-lg ${
                      highlightId === supplier.id ? 'ring-2 ring-blue-500 bg-blue-50' :
                  ...
                  >
                    ...
                    {isExpanded && (
                      <div className="border-t p-4 bg-gray-50">
                        {/* Sekce Detail o dodavateli - s rámečkem */}
                        <div className="mb-4 border border-gray-200 rounded-lg overflow-hidden">
                          <h4 className="font-semibold px-4 py-3 bg-gray-100 border-b">Detail o dodavateli</h4>
                          <div className="text-sm">
                            {(() => {
                              const entityType = supplier.entityType || 'company'
                              ...
                              return (
                                <>
                                  ...
                                  <div className="grid grid-cols-[1fr_auto_1fr] px-4 py-2 bg-white">
                                    ...
                                    {entityType === 'company' && (
                                      <div><span className="text-gray-600">Kontaktní osoba:</span> <span className="font-medium">{supplier.contact || '-'}</span></div>
                                    )}
                                  ...
                                  <div className="grid grid-cols-[1fr_auto_1fr] px-4 py-2 bg-gray-50">
                                    <div><span className="text-gray-600">Adresa:</span> <span className="font-medium">{supplier.address || '-'}</span></div>
                                    <div className="border-l border-gray-200 mx-4"></div>
                                    <div><span className="text-gray-600">Telefon:</span> <span className="font-medium">{supplier.phone || '-'}</span></div>
                                  </div>

                                  {/* Pro FIRMU: IČO a Email */}
                                  {entityType === 'company' && (
                                    <div className="grid grid-cols-[1fr_auto_1fr] px-4 py-2 bg-white">
                                      <div><span className="text-gray-600">IČO:</span> <span className="font-medium">{supplier.ico || '-'}</span></div>
                                      <div className="border-l border-gray-200 mx-4"></div>
                                      <div><span className="text-gray-600">Email:</span> <span className="font-medium">{supplier.email || '-'}</span></div>
                                    </div>
                                  )}
                                  ...
                                  {entityType === 'company' && (
                                    <div className="grid grid-cols-[1fr_auto_1fr] px-4 py-2 bg-gray-50">
                                      <div><span className="text-gray-600">DIČ:</span> <span className="font-medium">{supplier.dic || '-'}</span></div>
                                      <div className="border-l border-gray-200 mx-4"></div>
                                      <div><span className="text-gray-600">Web:</span> <span className="font-medium">{supplier.website || '-'}</span></div>
                                    </div>
                                  )}

                                  {/* Bankovní účet a Poznámka - vždy */}
                                  <div className="grid grid-cols-[1fr_auto_1fr] px-4 py-2 bg-white">
                                    <div><span className="text-gray-600">Bankovní účet:</span> <span className="font-medium">{supplier.bankAccount || '-'}</span></div>
                                    <div className="border-l border-gray-200 mx-4"></div>
                                    <div><span className="text-gray-600">Poznámka:</span> <span className="font-medium">{supplier.note || '-'}</span></div>
                                  </div>
                                </>
                              )
                            })()}
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-500 ml-2">
                      ({filteredSuppliers.length} celkem)
                    </span>
                  </div>
