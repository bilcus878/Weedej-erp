          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr className="text-xs text-gray-400 font-semibold uppercase tracking-wide">
                ...
                <th className="text-center px-3 py-3">Expirace</th>
                <th className="text-center px-3 py-3">Sklad</th>
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-3">
                      <span className="font-mono font-semibold text-amber-700">{batch.batchNumber}</span>
                      {batch.supplierLotRef && (
                        <p className="text-[10px] text-gray-400 mt-0.5">Lot dodavatele: {batch.supplierLotRef}</p>
                      )}
                    </td>
                    <td className="px-3 py-3">
                      <p className="font-medium text-gray-900">{batch.product?.name ?? '—'}</p>
                      <p className="text-[10px] text-gray-400">{batch.product?.unit}</p>
                    </td>
                    <td className="px-3 py-3 text-gray-600 text-xs">{batch.supplier?.name ?? '—'}</td>
                    <td className="text-center px-3 py-3 text-gray-600 text-xs">
                      {batch.receivedDate ? new Date(batch.receivedDate).toLocaleDateString('cs-CZ') : '—'}
                    </td>
                    <td className="text-center px-3 py-3">
                      {expiry ? (
                        <span className={`text-xs font-medium ${expired ? 'text-red-600' : expiryWarning ? 'text-orange-600' : 'text-gray-600'}`}>
                          {expiry.toLocaleDateString('cs-CZ')}
                          {daysLeft !== null && (
                            <span className="block text-[10px]">
                              {expired ? 'PROŠLÁ' : `${daysLeft}d`}
                            </span>
                          )}
                        </span>
                      ) : (
                    ...
                    <td className="text-center px-3 py-3">
                      <span className={`font-semibold text-sm ${(batch.currentStock ?? 0) > 0 ? 'text-gray-900' : 'text-gray-400'}`}>
                        {batch.currentStock ?? 0} {batch.product?.unit}
                      </span>
                    </td>
                    <td className="text-center px-3 py-3">
