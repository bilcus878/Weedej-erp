...
import { validateTransition, type ReturnStatus } from '@/lib/returns/returnWorkflow'
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    try {
      validateTransition(existing.status as ReturnStatus, toStatus as ReturnStatus)
    } catch (e: any) {
      return NextResponse.json({ error: e.message }, { status: 422 })
    }

    const closeTransitions = new Set(['closed', 'cancelled'])
    ...
    const updated = await prisma.$transaction(async tx => {
      const r = await tx.returnRequest.update({
        ...
        data:  {
          status:   toStatus,
          closedAt: closeTransitions.has(toStatus) ? new Date() : undefined,
