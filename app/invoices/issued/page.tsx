...
export default function TransactionsPage() {
  ...
  async function handleOpenForm() {
    ...
    // Získej další číslo faktury
    let nextInvoiceNumber = ''
    try {
      const res = await fetch('/api/transactions/next-invoice-number')
      const data = await res.json()
      nextInvoiceNumber = data.invoiceCode
    } catch (error) {
    ...
    // Přidej automaticky jednu prázdnou položku (jako u přijatých faktur)
    setTransactionItems([{
      productId: '',
      productName: '',
      isManual: false,
  ...
  function handleEditTransaction(transaction: Transaction) {
    ...
    setTransactionItems(transaction.items.map((item: any) => ({
  ...
  // Přidat/upravit fakturu
  async function handleAddTransaction(e: React.FormEvent) {
    e.preventDefault()

    if (transactionItems.length === 0) {
      alert('Přidej alespoň jeden produkt do faktury')
      return
    }

    // Vypočítej celkovou částku z položek
    ...
    if (totalAmount <= 0) {
      alert('Celková částka faktury musí být větší než 0')
      return
    }
    ...
    try {
      ...
      if (editingTransaction) {
        // Pokud editujeme, použij stejné číslo (nebo upravené z formuláře)
        transactionCode = formData.invoiceNumber || editingTransaction.transactionCode
      } else {
        // Pro novou fakturu použij číslo z formuláře nebo vygeneruj nové
        if (formData.invoiceNumber) {
          transactionCode = formData.invoiceNumber
        } else {
          // Pro novou fakturu získej další FA číslo
          const invoiceNumRes = await fetch('/api/transactions/next-invoice-number')
          const { invoiceCode } = await invoiceNumRes.json()
          transactionCode = invoiceCode
        }
      }

      // Připrav položky s jednotkami a cenami
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          customerId: isCustomerManual ? null : (formData.customerId || null),
          customerName: isCustomerManual ? customerName : null,
          customerICO: isCustomerManual ? customerICO : null,
          customerDIC: isCustomerManual ? customerDIC : null,
          customerAddress: isCustomerManual ? customerAddress : null,
          note: note || null,
          items,
        }),
      })
      ...
      if (response.ok) {
        alert(editingTransaction ? 'Faktura upravena!' : 'Faktura vytvořena!')
        setShowForm(false)
        setEditingTransaction(null)
        setFormData({
          invoiceNumber: '',
