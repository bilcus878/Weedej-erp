...
export function OrderItemsSection({ order, isVatPayer, title = 'Položky' }: OrderItemsSectionProps) {
  ...
  // ── Single data pass — compute display values once, used by both mobile and desktop ──
  const computedItems = catalogItems.map((item, i) => {
    ...
    return { item, qty, qtyDisplay, unitPrice, vatRate, vatPerUnit, priceWithVat, rowTotal, invMovId, invLink, rowBg }
  })

  const catalogSubtotal = computedItems.reduce((sum, { rowTotal }) => sum + rowTotal, 0)
  const hasExtras       = shippingTotal !== 0 || discountTotal !== 0

  return (
    <ERPSectionCard title={headerTitle} icon={<Package />} className="overflow-hidden">

      {/* ── Mobile: stacked semantic blocks (< md) ───────────────────────── */}
      {/* No horizontal scroll — each item is self-contained */}
      <div className="-mx-5 -my-4 md:hidden divide-y divide-gray-100">
        {computedItems.map(({ item, qtyDisplay, unitPrice, vatRate, rowTotal, invMovId, invLink }) => (
          <div key={item.id} className="px-5 py-3 space-y-1">
            ...
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs text-gray-500">
                {qtyDisplay} × {formatPrice(unitPrice)}
                {isVatPayer && <span className="ml-1.5 text-gray-400">DPH {vatRate}%</span>}
              </span>
              <span className="text-sm font-bold text-gray-900 tabular-nums">{formatPrice(rowTotal)}</span>
            </div>
            {invMovId && <div className="pt-0.5">{invLink}</div>}
          </div>
        ))}

        {/* Shipping / discount lines */}
        {shippingTotal !== 0 && (
          <div className="px-5 py-2.5 flex justify-between items-center bg-blue-50/60">
            <span className="text-sm text-gray-700">{shippingItem?.productName || 'Doprava'}</span>
            <span className="text-sm font-medium text-blue-700 tabular-nums">{formatPrice(shippingTotal)}</span>
          </div>
        )}
        {discountTotal !== 0 && (
          <div className="px-5 py-2.5 flex justify-between items-center bg-yellow-50/60">
            <span className="text-sm text-gray-700">{discountItem?.productName || 'Sleva'}</span>
            <span className="text-sm font-medium text-red-600 tabular-nums">{formatPrice(discountTotal)}</span>
          </div>
        )}

        {/* Grand total */}
        <div className="px-5 py-3 flex justify-between items-center bg-gray-100 font-bold text-sm">
          <span>{isVatPayer ? 'Celkem s DPH' : 'Celkem'}</span>
          <span className="tabular-nums">{formatPrice(Number(order.totalAmount))}</span>
        </div>
      </div>

      {/* ── Desktop: full data grid (≥ md) ───────────────────────────────── */}
      <div className="-mx-5 -my-4 hidden md:block overflow-x-auto">
        <div className="min-w-[640px]">
          ...
          {/* Item rows */}
          {computedItems.map(({ item, qtyDisplay, unitPrice, vatRate, vatPerUnit, priceWithVat, rowTotal, invLink, rowBg }) =>
            isVatPayer ? (
          ...
          {/* Subtotals — only shown when shipping or discount exists */}
          {hasExtras && (() => {
            ...
            const align     = isVatPayer ? 'text-center' : 'text-right'
            return (
              <>
                <div className={`grid ${colGrid} gap-2 px-5 py-2 bg-gray-50 border-t border-gray-100 text-sm`}>
                  ...
                  <div className={`${align} font-medium text-gray-800`}>{formatPrice(catalogSubtotal)}</div>
                </div>
                {shippingTotal !== 0 && (
                  <div className={`grid ${colGrid} gap-2 px-5 py-2 bg-blue-50/60 border-t border-blue-100 text-sm`}>
                    ...
                    <div className={`${align} text-blue-700 font-medium`}>{formatPrice(shippingTotal)}</div>
                  </div>
                )}
                {discountTotal !== 0 && (
                  <div className={`grid ${colGrid} gap-2 px-5 py-2 bg-yellow-50/60 border-t border-yellow-100 text-sm`}>
                    ...
                    <div className={`${align} text-red-600 font-medium`}>{formatPrice(discountTotal)}</div>
                  </div>
                )}
              </>
            )
