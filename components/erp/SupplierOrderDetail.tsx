...
export interface SupplierOrderDetailItem {
  ...
  quantity: number
  alreadyReceivedQuantity?: number  // tracks receipt progress
  unit: string
  price: number         // bez DPH (expectedPrice)
  vatRate: number
  vatAmount: number     // per unit
  priceWithVat: number  // s DPH per unit
...
interface Props {
  ...
  isVatPayer: boolean
  orderHref?: string   // link to related purchase order (used from invoice page)
  onPrintPdf?: () => void
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="flex-1 px-4 text-sm bg-white divide-y divide-gray-100">
            ...
            <div className="py-3 space-y-2.5">
              ...
              <div className="flex justify-between items-center gap-2">
                ...
                <span className="font-medium text-gray-800 text-right">{order.supplierPhone || <span className="text-gray-400">—</span>}</span>
              </div>
            </div>
            {/* Firemní údaje */}
            <div className="py-3 space-y-2.5">
              ...
              {order.supplierWebsite && (
                <div className="flex justify-between items-center gap-2">
                  ...
                  <span className="font-medium text-gray-800 text-right text-xs">{order.supplierWebsite}</span>
                </div>
              )}
              {!order.supplierICO && !order.supplierDIC && !order.supplierAddress && !order.supplierWebsite && (
                <p className="text-xs text-gray-400 italic">Firemní údaje nejsou k dispozici</p>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="flex-1 px-4 text-sm bg-white divide-y divide-gray-100 flex flex-col">
            ...
            <div className="py-3 space-y-2.5">
              ...
              <div className="flex justify-between items-center gap-2">
                ...
                <span className="font-medium text-gray-800 text-right">
                  {order.receipts?.find(r => r.status !== 'storno')
            ...
            <div className="py-3 space-y-2.5 mt-auto">
              {order.supplierBankAccount && (
                <div className="flex justify-between items-center gap-2">
                  ...
                  <span className="font-mono text-xs text-gray-700 text-right break-all">{order.supplierBankAccount}</span>
                </div>
              )}
              {order.note && (
                <div className="flex justify-between items-center gap-2">
        ...
        <div className="md:col-span-2 border border-gray-200 rounded-lg overflow-hidden">
          ...
          <div className="grid grid-cols-3 divide-x divide-gray-200 bg-white text-sm">
