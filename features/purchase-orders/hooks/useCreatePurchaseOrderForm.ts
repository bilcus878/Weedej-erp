...
export function useCreatePurchaseOrderForm(
  ...
  async function handleOpen() {
    const nextNumber    = await fetchNextOrderNumber(orderDate)
    ...
    setItems(prev => {
      ...
      if (field === 'productId') {
        ...
        if (product) {
          ...
          next[index] = { ...next[index], productId: value, productName: '', isManual: false, unit: product.unit, expectedPrice: product.purchasePrice || 0, vatRate: effectiveVatRate }
          return next
        }
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isManualSupplier && !manualSupplierData.name.trim()) {
      alert('Vyplňte alespoň název dodavatele'); return
    }
    ...
    try {
      await createPurchaseOrder({
        ...
        constantSymbol: constantSymbol || null, specificSymbol: specificSymbol || null,
        note, isManualSupplier, isAnonymousSupplier, saveSupplierToDatabase,
        ...
        items: items.map(item => ({
          ...
          isManual:    item.isManual, quantity: item.quantity,
          unit:        item.unit, expectedPrice: item.expectedPrice,
        })),
      })
      handleClose()
      await onSuccess()
  ...
  return {
    open, orderNumber, orderDate, supplierId, setSupplierId, expectedDate, setExpectedDate,
    note, setNote, items, dueDate, setDueDate, paymentType, setPaymentType,
    variableSymbol, setVariableSymbol, constantSymbol, setConstantSymbol,
    specificSymbol, setSpecificSymbol, isManualSupplier, setIsManualSupplier,
    isAnonymousSupplier, setIsAnonymousSupplier, saveSupplierToDatabase, setSaveSupplierToDatabase,
    manualSupplierData, setManualSupplierData,
    handleOpen, handleClose, handleOrderDateChange, handleAddItem, handleRemoveItem, handleItemChange, handleSubmit,
  }
}
