...
export function PurchaseOrderTotalsPreview({ items, isVatPayer, discountType, discountValue }: Props) {
  if (items.length === 0) return null

  const fixedDiscount = discountType === 'fixed' && discountValue ? parseFloat(discountValue) : 0

  if (!isVatPayer) {
    const subtotal = items.reduce((s, i) => s + ((i.quantity || 0) * (i.expectedPrice || 0)), 0)
  ...
  const vatLineItems = items.map(i => calculateLineVat(i.quantity || 0, i.expectedPrice || 0, i.vatRate))
