...
export async function calculateReservedStock(productId: string): Promise<number> {
  const { calculateReservedStock: calc } = await import('@/lib/shared/inventory/stockMovement')
...
export async function canReserveQuantity(
  ...
  const { calculateCurrentStock } = await import('@/lib/shared/inventory/stockMovement')
